...
public class GameController {
    ...
    @GetMapping("/rawg/{id}")
    public GameDTO getGameFromApi(@PathVariable String id) { return gameService.getGameFromApi(id); }

    @GetMapping("/rawg/top-rated")
    public List<GameDTO> getTopRated(@RequestParam(defaultValue = "1") int page,
                                     @RequestParam(defaultValue = "8") int pageSize) {
        return gameService.getTopRatedPopular(page, pageSize);
    }

    @GetMapping("/rawg/coming-soon")
    public List<GameDTO> getComingSoon(@RequestParam(defaultValue = "1") int page,
                                       @RequestParam(defaultValue = "8") int pageSize) {
        return gameService.getComingSoonPopular(page, pageSize);
    }

    @GetMapping("/rawg/featured")
    public List<GameDTO> getFeaturedGames(@RequestParam(defaultValue = "1") int page,
